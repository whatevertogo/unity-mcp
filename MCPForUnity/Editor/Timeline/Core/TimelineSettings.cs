...
{
    /// <summary>
    /// Persistent settings for the Timeline system.
    ...
    {
        private const string SettingsPath = "Assets/TimelineSettings.asset";

        private static TimelineSettings _instance;
        ...
        private static TimelineSettings LoadSettings()
        {
            return AssetDatabase.LoadAssetAtPath<TimelineSettings>(SettingsPath);
        }

        private static TimelineSettings CreateSettings()
        {
            var settings = CreateInstance<TimelineSettings>();
            AssetDatabase.CreateAsset(settings, SettingsPath);
            AssetDatabase.SaveAssets();
            AssetDatabase.Refresh();
            Debug.Log($"[TimelineSettings] Created new settings at {SettingsPath}");
            return settings;
        }
        ...
        public void Save()
        {
            ...
            AssetDatabase.SaveAssets();
            Debug.Log("[TimelineSettings] Settings saved");
        }
    ...
    /// <summary>
    /// Custom editor for TimelineSettings.
    /// Provides a clean UI for modifying timeline settings.
    /// </summary>
    [CustomEditor(typeof(TimelineSettings))]
    public class TimelineSettingsEditor : UnityEditor.Editor
    {
        public override void OnInspectorGUI()
        {
            ...
            EditorGUILayout.PropertyField(so.FindProperty(nameof(TimelineSettings.MinImportanceForRecording)),
            ...
            EditorGUILayout.PropertyField(so.FindProperty(nameof(TimelineSettings.EnableEventMerging)),
                new GUIContent("Enable Event Merging", "Merge high-frequency events within time window"));
            EditorGUILayout.PropertyField(so.FindProperty(nameof(TimelineSettings.MergeWindowMs)),
            ...
            EditorGUILayout.PropertyField(so.FindProperty(nameof(TimelineSettings.MaxEvents)),
                new GUIContent("Max Events", "Maximum number of events to store"));
            EditorGUILayout.PropertyField(so.FindProperty(nameof(TimelineSettings.HotEventCount)),
            ...
            if (GUI.changed)
            {
                (target as TimelineSettings)?.Save();
            }
        }

        private void SetImportance(float value)
        {
            SerializedProperty prop = serializedObject.FindProperty(nameof(TimelineSettings.MinImportanceForRecording));
