...
using MCPForUnity.Editor.Timeline.Helpers;
            ...
            string toolName)
        {
            ...
            if (!string.IsNullOrEmpty(toolCallId))
            {
                ...
                string displayName = string.IsNullOrEmpty(toolName)
                    ? "AI operation"
                    : TimelineHelper.FormatToolName(toolName);

                return $"{displayName}: {batch.Count} events in {TimelineHelper.FormatDurationFromRange(batch[0].TimestampUnixMs, batch[batch.Count - 1].TimestampUnixMs)}";
            }
