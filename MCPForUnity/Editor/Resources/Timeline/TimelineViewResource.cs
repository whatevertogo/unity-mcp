...
namespace MCPForUnity.Editor.Resources.Timeline
{
    ...
    public static class TimelineViewResource
    {
        ...
        /// </summary>
        private static List<TimelineViewItem> ApplyTaskFiltersToProjected(List<TimelineViewItem> projected, string taskId, string conversationId)
        {
            if (string.IsNullOrEmpty(taskId) && string.IsNullOrEmpty(conversationId))
                return projected;
        ...
        private static object QueryAggregated(int limit, long? sinceSequence, float minImportance, string taskId, string conversationId)
        {
            ...
            {
                schema_version = "timeline_view@4",
                items = items,
                total = items.Length,
                current_sequence = EventStore.CurrentSequence
            });
